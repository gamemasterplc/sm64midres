...
void show_crash_screen_and_hang(void) {
    ...
    if (nAssertStopProgram == 0) {
        fbFillColor = 0x6253;
        fb_fill(10, 10, SCREEN_WIDTH-20, SCREEN_HEIGHT-20);

        fb_print_str((SCREEN_WIDTH/2)-80, 20, "AN ERROR HAS OCCURRED!");
        fb_print_int_hex((SCREEN_WIDTH/2)-80, 30, errno, 8);
        fb_print_str((SCREEN_WIDTH/2)-53, 30, szErrCodes[errno]);
        ...
        if (errno >= 2 && errno <= 5) {
            ...
            */
            ...
            fb_print_str((SCREEN_WIDTH/2)+28, 50, "VA");
        ...
        fb_print_str((SCREEN_WIDTH/2)-80, 20, "ASSERTION FAILED!");

        afterFileX = fb_print_str((SCREEN_WIDTH/2)-80, 30, pAssertFile);
        fb_print_str(afterFileX, 30, ":");
        fb_print_uint(afterFileX + 5, 30, nAssertLine);

        exprBoxWidth = (crash_strlen(pAssertExpression) * 5) + 2;
        fbFillColor = 0x0001;
        fb_fill((SCREEN_WIDTH/2)-80 - 1, 40 - 1, exprBoxWidth, 10);
        fb_print_str((SCREEN_WIDTH/2)-80, 40, pAssertExpression);
    }

    fb_print_str((SCREEN_WIDTH/2)-80, 50, "PC");
    fb_print_int_hex(95, 50, epc, 32);

    fb_print_gpr_states((SCREEN_WIDTH/2)-80, 70, szGPRegisters1, &exceptionRegContext[6 + 0]);
    fb_print_gpr_states((SCREEN_WIDTH/2)-15, 70, szGPRegisters2, &exceptionRegContext[6 + 15 * 2]);
