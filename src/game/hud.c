...
static void animate_power_meter_deemphasizing(void) {
    ...
    if (sPowerMeterHUD.y >= (SCREEN_HEIGHT-59)) {
        speed = 3;
    }

    if (sPowerMeterHUD.y >= (SCREEN_HEIGHT-49)) {
        speed = 2;
    }

    if (sPowerMeterHUD.y >= (SCREEN_HEIGHT-44)) {
        speed = 1;
    }

    sPowerMeterHUD.y += speed;

    if (sPowerMeterHUD.y >= (SCREEN_HEIGHT-39)) {
        sPowerMeterHUD.y = (SCREEN_HEIGHT-40);
...
static void animate_power_meter_hiding(void) {
    sPowerMeterHUD.y += 20;
    if (sPowerMeterHUD.y >= (SCREEN_HEIGHT+61)) {
...
void handle_power_meter_actions(s16 numHealthWedges) {
    ...
    if (numHealthWedges < 8 && sPowerMeterStoredHealth == 8 && sPowerMeterHUD.animation == POWER_METER_HIDDEN) {
        ...
        sPowerMeterHUD.y = (SCREEN_HEIGHT-74);
    }
...
#ifdef VERSION_JP
#define HUD_TOP_Y SCREEN_HEIGHT-30
#else
#define HUD_TOP_Y SCREEN_HEIGHT-31
#endif
...
void render_hud_coins(void) {
    print_text(((SCREEN_WIDTH/2)+8), HUD_TOP_Y, "+"); // 'Coin' glyph
    print_text(((SCREEN_WIDTH/2)+24), HUD_TOP_Y, "*"); // 'X' glyph
    print_text_fmt_int(((SCREEN_WIDTH/2)+38), HUD_TOP_Y, "%d", gHudDisplay.coins);
}

#ifdef VERSION_JP
#define HUD_STARS_X (SCREEN_WIDTH-73)
#else
#define HUD_STARS_X (SCREEN_WIDTH-78)
#endif
...
void render_hud_keys(void) {
    ...
    for (i = 0; i < gHudDisplay.keys; i++) {
        print_text((i * 16) + (SCREEN_WIDTH-100), ((SCREEN_HEIGHT/2)+22), "/"); // unused glyph - beta key
    }
}
...
void render_hud_timer(void) {
    ...
    switch (eu_get_language()) {
        case LANGUAGE_ENGLISH:
            print_text((SCREEN_WIDTH-150), (SCREEN_HEIGHT-55), "TIME");
            break;
        case LANGUAGE_FRENCH:
            print_text((SCREEN_WIDTH-155), (SCREEN_HEIGHT-55), "TEMPS");
            break;
        case LANGUAGE_GERMAN:
            print_text((SCREEN_WIDTH-150), (SCREEN_HEIGHT-55), "ZEIT");
            break;
    }
#endif
    ...
    print_text((SCREEN_WIDTH-150), (SCREEN_HEIGHT-55), "TIME");
#endif
    print_text_fmt_int((SCREEN_WIDTH-91), (SCREEN_HEIGHT-55), "%0d", timerMins);
    print_text_fmt_int((SCREEN_WIDTH-71), (SCREEN_HEIGHT-55), "%02d", timerSecs);
    print_text_fmt_int((SCREEN_WIDTH-37), (SCREEN_HEIGHT-55), "%d", timerFracSecs);
    gSPDisplayList(gDisplayListHead++, dl_hud_img_begin);
    render_hud_tex_lut((SCREEN_WIDTH-81), 32, (*hudLUT)[GLYPH_APOSTROPHE]);
    render_hud_tex_lut((SCREEN_WIDTH-46), 32, (*hudLUT)[GLYPH_DOUBLE_QUOTE]);
