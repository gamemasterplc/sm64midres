...
void draw_profiler_bar(s64 clock_base, s64 clock_start, s64 clock_end, s16 pos_y, u16 color) {
    ...
    if (rect_x1 > SCREEN_WIDTH-1) {
        clock_start = SCREEN_WIDTH-1;
    }
    if (rect_x2 > SCREEN_WIDTH-1) {
        clock_end = SCREEN_WIDTH-1;
    }
...
void draw_reference_profiler_bars(void) {
    ...
    gDPFillRectangle(gDisplayListHead++, 30, SCREEN_HEIGHT-20, 79, SCREEN_HEIGHT-18);

    // Yellow
    gDPPipeSync(gDisplayListHead++);
    gDPSetFillColor(gDisplayListHead++,
                    GPACK_RGBA5551(255, 255, 40, 1) << 16 | GPACK_RGBA5551(255, 255, 40, 1));
    gDPFillRectangle(gDisplayListHead++, 79, SCREEN_HEIGHT-20, 128, SCREEN_HEIGHT-18);

    // Orange
    gDPPipeSync(gDisplayListHead++);
    gDPSetFillColor(gDisplayListHead++,
                    GPACK_RGBA5551(255, 120, 40, 1) << 16 | GPACK_RGBA5551(255, 120, 40, 1));
    gDPFillRectangle(gDisplayListHead++, 128, SCREEN_HEIGHT-20, 177, SCREEN_HEIGHT-18);

    // Red
    gDPPipeSync(gDisplayListHead++);
    gDPSetFillColor(gDisplayListHead++,
                    GPACK_RGBA5551(255, 40, 40, 1) << 16 | GPACK_RGBA5551(255, 40, 40, 1));
    gDPFillRectangle(gDisplayListHead++, 177, SCREEN_HEIGHT-20, 226, SCREEN_HEIGHT-18);
}
...
void draw_profiler_mode_1(void) {
    ...
    draw_profiler_bar(clock_base, profiler->gameTimes[0], profiler->gameTimes[1], SCREEN_HEIGHT-28,
    ...
    draw_profiler_bar(clock_base, profiler->gameTimes[1], profiler->gameTimes[2], SCREEN_HEIGHT-28,
                      GPACK_RGBA5551(255, 120, 40, 1));

    // draw the profiler for the time it takes for the display lists to send. (blue)
    draw_profiler_bar(clock_base, profiler->gameTimes[2], profiler->gameTimes[3], SCREEN_HEIGHT-28,
    ...
    for (i = 0; i < profiler->numSoundTimes; i += 2) {
        draw_profiler_bar(clock_base, profiler->soundTimes[i], profiler->soundTimes[i + 1], SCREEN_HEIGHT-28,
    ...
    //  this.
    draw_profiler_bar(clock_base, profiler->gfxTimes[0], profiler->gfxTimes[1], SCREEN_HEIGHT-24,
                      GPACK_RGBA5551(255, 255, 40, 1));
    // (orange)
    draw_profiler_bar(clock_base, profiler->gfxTimes[1], profiler->gfxTimes[2], SCREEN_HEIGHT-24,
                      GPACK_RGBA5551(255, 120, 40, 1));
    ...
    for (i = 0; i < profiler->numVblankTimes; i += 2) {
        draw_profiler_bar(clock_base, profiler->vblankTimes[i], profiler->vblankTimes[i + 1], SCREEN_HEIGHT-24,
                          GPACK_RGBA5551(255, 40, 40, 1));
    }
...
void draw_profiler_mode_0(void) {
    ...
    clock_start = 0;
    draw_profiler_bar(0, clock_start, clock_start + task_start, SCREEN_HEIGHT-28, GPACK_RGBA5551(255, 40, 40, 1));

    // draw level script execution duration. (yellow)
    clock_start += task_start;
    draw_profiler_bar(0, clock_start, clock_start + level_script_duration, SCREEN_HEIGHT-28,
                      GPACK_RGBA5551(255, 255, 40, 1));

    // draw render duration. (orange)
    clock_start += level_script_duration;
    draw_profiler_bar(0, clock_start, clock_start + render_duration, SCREEN_HEIGHT-28,
                      GPACK_RGBA5551(255, 120, 40, 1));

    // Draw bottom profilers.

    // rdp duration (orange)
    draw_profiler_bar(0, 0, rdp_duration, SCREEN_HEIGHT-24, GPACK_RGBA5551(255, 120, 40, 1));
    // rsp duration (yellow)
    draw_profiler_bar(0, 0, rsp_duration, SCREEN_HEIGHT-24, GPACK_RGBA5551(255, 255, 40, 1));
    // vblank duration (red)
    draw_profiler_bar(0, 0, vblank, SCREEN_HEIGHT-24, GPACK_RGBA5551(255, 40, 40, 1));
