...
void PatchDialogTable(void **table)
{
	void *ptr = table[0];
	int i = 0;
	int x;
	...
	while(ptr)
	{
		dialog = segmented_to_virtual(ptr);
		x = dialog->leftOffset;
		y = dialog->width;
		if(x == 95)
		{
			dialog->leftOffset = ((SCREEN_WIDTH/2)-65);
		}
		if(x == 150)
		{
			dialog->leftOffset = SCREEN_WIDTH-170;
		}
		if(y == 200)
		{
			dialog->width = SCREEN_HEIGHT-40;
		}
		i++;
		ptr = table[i];
	}
...
s32 lvl_init_from_save_file(UNUSED s16 arg0, s32 levelNum) {
    ...
    switch (var) {
        case LANGUAGE_ENGLISH:
			...
			#if SCREEN_WIDTH != 320 || SCREEN_HEIGHT != 240
			PatchDialogTable(segmented_to_virtual(dialog_table_eu_en));
			#endif
            break;
        case LANGUAGE_FRENCH:
			...
			#if SCREEN_WIDTH != 320 || SCREEN_HEIGHT != 240
			PatchDialogTable(segmented_to_virtual(dialog_table_eu_fr));
			#endif
            break;
        case LANGUAGE_GERMAN:
			...
			#if SCREEN_WIDTH != 320 || SCREEN_HEIGHT != 240
			PatchDialogTable(segmented_to_virtual(dialog_table_eu_de));
			#endif
            break;
    }
#else
	#if SCREEN_WIDTH != 320 || SCREEN_HEIGHT != 240
	PatchDialogTable(segmented_to_virtual(seg2_dialog_table));
	#endif
#endif
    sWarpDest.type = WARP_TYPE_NOT_WARPING;
    sDelayedWarpOp = WARP_OP_NONE;
