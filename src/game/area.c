...
void play_transition(s16 transType, s16 time, u8 red, u8 green, u8 blue) {
    ...
    if (transType < 8) {
        ...
        // across the screen.
        gWarpTransition.data.startCircleX = (SCREEN_WIDTH/2);
        gWarpTransition.data.startCircleY = (SCREEN_HEIGHT/2);
        gWarpTransition.data.endCircleX = (SCREEN_WIDTH/2);
        gWarpTransition.data.endCircleY = (SCREEN_HEIGHT/2);

        gWarpTransition.data.unk10 = 0;

        if (transType & 1) // Is the image fading in?
        {
            gWarpTransition.data.startCircleRadius = SCREEN_WIDTH;
            if (transType >= 0x0F) {
        ...
        } else // The image is fading out. (Reverses start & end circles)
        {
            ...
            gWarpTransition.data.endCircleRadius = SCREEN_WIDTH;
        }
    }
}
