...
#if SCREEN_WIDTH != 320 || SCREEN_HEIGHT != 240
void EditVIModeforResolution(OSViMode *mode, int width, int height)
{
...
void thread1_idle(UNUSED void *arg) {
	...
	#else
	if (sp24 == TV_TYPE_NTSC){
		gNewViMode = osViModeTable[OS_VI_NTSC_LAN1];
	} else {
		gNewViMode = osViModeTable[OS_VI_PAL_LAN1];
	}
	EditVIModeforResolution(&gNewViMode, SCREEN_WIDTH, SCREEN_HEIGHT);
	osViSetMode(&gNewViMode);
	#endif
#else
	#if SCREEN_WIDTH == 320 && SCREEN_HEIGHT == 240
    osViSetMode(&osViModeTable[OS_VI_NTSC_LAN1]);
