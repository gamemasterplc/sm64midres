...
struct ObjView *make_view_withgrp(char *name, struct ObjGroup *grp) {
    ...
    view = make_view(name, (VIEW_DRAW | VIEW_ALLOC_ZBUF | VIEW_MOVEMENT), 1, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, grp);
...
void gd_draw_rect(f32 ulx, f32 uly, f32 lrx, f32 lry) {
    ...
    gDPSetRenderMode(next_gfx(), G_RM_RA_ZB_OPA_INTER, G_RM_NOOP2);
}
...
void Unknown801A5C80(struct ObjGroup *parentGroup) {
    ...
    d_set_rel_pos(10.0f, SCREEN_HEIGHT-10, 0.0f);
