...
s16 level_select_input_loop(void) {
    ...
    gCurrActNum = 6;
    print_text_centered((SCREEN_WIDTH/2), 80, "SELECT STAGE");
    print_text_centered((SCREEN_WIDTH/2), 30, "PRESS START BUTTON");
    print_text_fmt_int(((SCREEN_WIDTH/2)-120), 60, "%2d", gCurrLevelNum);
    print_text(((SCREEN_WIDTH/2)-80), 60, gLevelSelect_StageNamesText[gCurrLevelNum - 1]); // print stage name
