...
Gfx *geo18_fade_transition(u32 sp40, struct GraphNode *sp44, UNUSED u32 sp48) {
    ...
    if (sp40 != 1) {
        ...
        if (gTitleFadeCounter == 0xFF) {
            ...
            graphNode->flags = (graphNode->flags & 0xFF) | 0x100;
            gDPSetRenderMode(displayListIter++, G_RM_RA_OPA_SURF, G_RM_RA_OPA_SURF2);
        } else {
...
Gfx *intro_backdrop_one_image(u32 index, s8 *backgroundTable) {
	...
	int tiles_per_row = ((SCREEN_WIDTH+79)/80);
	int tile_offset_x = -((SCREEN_WIDTH%80)/2);
	int tile_offset_y = -((SCREEN_HEIGHT%80)/2);
	#endif
	...
	guTranslate(mtx, (tile_offset_x+(((s32)index%tiles_per_row)*80)), (tile_offset_y+(((s32)index/tiles_per_row)*80)), 0.0f);
	#endif
...
Gfx *geo18_intro_backdrop(u32 sp48, struct GraphNode *sp4c, UNUSED u32 sp50) {
    ...
    displayListIter = NULL;
    if (sp48 == 1) {
        displayList = alloc_display_list(4+(((SCREEN_WIDTH+79)/80)*((SCREEN_HEIGHT+159)/80)) * sizeof(*displayList));
        displayListIter = displayList;
        graphNode->node.flags = (graphNode->node.flags & 0xFF) | 0x100;
        gSPDisplayList(displayListIter++, &dl_proj_mtx_fullscreen);
        gSPDisplayList(displayListIter++, &title_screen_bg_dl_0A000100);
        for (i = 0; i < (((SCREEN_WIDTH+79)/80)*((SCREEN_HEIGHT+159)/80)); ++i) {
...
Gfx *geo18_game_over_tile(u32 sp40, struct GraphNode *sp44, UNUSED u32 sp48) {
    ...
    if (sp40 != 1) {
        ...
        gSPDisplayList(displayListIter++, &dl_proj_mtx_fullscreen);
        gSPDisplayList(displayListIter++, &title_screen_bg_dl_0A000100);
        for (j = 0; j < (s32) (((SCREEN_WIDTH+79)/80)*((SCREEN_HEIGHT+159)/80)); ++j) {
