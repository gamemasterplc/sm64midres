...
#define SCALE_POS_X(x) ((((x-160)*SCREEN_HEIGHT)/240)+(SCREEN_WIDTH/2))
#define SCALE_POS_Y(y) ((((y-120)*SCREEN_HEIGHT)/240)+(SCREEN_HEIGHT/2))
...
static s32 check_clicked_button(s16 x, s16 y, f32 depth) {
    f32 a = 52.4213;
    f32 newX = ((f32) x * ((SCREEN_HEIGHT*2)/3)) / (a * depth);
    f32 newY = ((f32) y * (SCREEN_HEIGHT/2)) / (a * 3.0f / 4.0f * depth);
    s16 maxX = newX + ((SCREEN_HEIGHT*25)/240);
    s16 minX = newX - ((SCREEN_HEIGHT*25)/240);
    s16 maxY = newY + ((SCREEN_HEIGHT*21)/240);
    s16 minY = newY - ((SCREEN_HEIGHT*21)/240);
...
static void handle_controller_cursor_input(void) {
    ...
    if (sCursorPos[0] > (((f32)SCREEN_WIDTH/2)-28)) {
        sCursorPos[0] = (((f32)SCREEN_WIDTH/2)-28);
    }
    if (sCursorPos[0] < -(((f32)SCREEN_WIDTH/2)-28)) {
        sCursorPos[0] = -(((f32)SCREEN_WIDTH/2)-28);
    }

    if (sCursorPos[1] > (((f32)SCREEN_HEIGHT/2)-30)) {
        sCursorPos[1] = (((f32)SCREEN_HEIGHT/2)-30);
    }
    if (sCursorPos[1] < -(((f32)SCREEN_HEIGHT/2)-30)) {
        sCursorPos[1] = -(((f32)SCREEN_HEIGHT/2)-30);
    }
...
static void print_menu_cursor(void) {
    ...
    create_dl_translation_matrix(MENU_MTX_PUSH, sCursorPos[0] + (SCREEN_WIDTH/2) - 5.0, sCursorPos[1] + (SCREEN_HEIGHT/2) - 25.0, 0.0f);
...
#ifdef VERSION_JP
#define SELECT_FILE_X SCALE_POS_X(160)-64
#define SCORE_X SCALE_POS_X(65)-15
#define COPY_X SCALE_POS_X(127)-12
#define ERASE_X SCALE_POS_X(195)-15
#define SOUNDMODE_X1 SCALE_POS_X(254)-19
#else
#define SELECT_FILE_X SCALE_POS_X(160)-67
#define SCORE_X SCALE_POS_X(67)-15
#define COPY_X SCALE_POS_X(129)-12
#define ERASE_X SCALE_POS_X(192)-15
...
static void print_main_menu_strings(void) {
    ...
    print_hud_lut_string(HUD_LUT_DIFF, SELECT_FILE_X, SCALE_POS_Y(43)-8, textSelectFile);
    // Print file star counts
    print_save_file_star_count(SAVE_FILE_A, SCALE_POS_X(92), SCALE_POS_Y(94)-16);
    print_save_file_star_count(SAVE_FILE_B, SCALE_POS_X(209), SCALE_POS_Y(94)-16);
    print_save_file_star_count(SAVE_FILE_C, SCALE_POS_X(92), SCALE_POS_Y(134)-16);
    print_save_file_star_count(SAVE_FILE_D, SCALE_POS_X(209), SCALE_POS_Y(134)-16);
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
   // Print menu names
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_generic_string(SCORE_X, SCALE_POS_Y(39), textScore);
    print_generic_string(COPY_X, SCALE_POS_Y(39), textCopy);
    print_generic_string(ERASE_X, SCALE_POS_Y(39), textErase);
#ifndef VERSION_JP
    sSoundTextX = get_str_x_pos_from_center(SCALE_POS_X(254), textSoundModes[sSoundMode], 10.0f);
#endif
    print_generic_string(SOUNDMODE_X1, SCALE_POS_Y(39), textSoundModes[sSoundMode]);
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_end);
    // Print file names
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_menu_generic_string(SCALE_POS_X(92), SCALE_POS_Y(65), textMarioA);
    print_menu_generic_string(SCALE_POS_X(207), SCALE_POS_Y(65), textMarioB);
    print_menu_generic_string(SCALE_POS_X(92), SCALE_POS_Y(105), textMarioC);
    print_menu_generic_string(SCALE_POS_X(207), SCALE_POS_Y(105), textMarioD);
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_end);
}

#ifdef VERSION_JP
#define CHECK_FILE_X ((SCREEN_WIDTH/2)-70)
#define NOSAVE_DATA_X1 ((SCREEN_WIDTH/2)-70)
#else
#define CHECK_FILE_X ((SCREEN_WIDTH/2)-65)
#define NOSAVE_DATA_X1 ((SCREEN_WIDTH/2)-61)
#endif
...
static void score_menu_display_message(s8 messageID) {
    switch (messageID) {
        case SCORE_MSG_CHECK_FILE:
            print_hud_lut_string_fade(HUD_LUT_DIFF, CHECK_FILE_X, SCALE_POS_Y(43)-8, textCheckFile);
            break;
        case SCORE_MSG_NOSAVE_DATA:
            print_generic_string_fade(NOSAVE_DATA_X1, SCALE_POS_Y(197)-7, textNoSavedDataExists);
            break;
    }
}

#ifdef VERSION_JP
#define RETURN_X SCALE_POS_X(63)-18
#define COPYFILE_X1 SCALE_POS_X(160)-32
#define ERASEFILE_X1 SCALE_POS_X(257)-29
#else
#define RETURN_X SCALE_POS_X(63)-19
#define COPYFILE_X1 SCALE_POS_X(160)-25
#define ERASEFILE_X1 SCALE_POS_X(257)-26
#endif
...
static void print_score_menu_strings(void) {
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_save_file_star_count(SAVE_FILE_A, SCALE_POS_X(90), SCALE_POS_Y(94)-18);
    print_save_file_star_count(SAVE_FILE_B, SCALE_POS_X(211), SCALE_POS_Y(94)-18);
    print_save_file_star_count(SAVE_FILE_C, SCALE_POS_X(90), SCALE_POS_Y(134)-15);
    print_save_file_star_count(SAVE_FILE_D, SCALE_POS_X(211), SCALE_POS_Y(134)-15);
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
    // Print menu names
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_generic_string(RETURN_X, SCALE_POS_Y(35), textReturn);
    print_generic_string(COPYFILE_X1, SCALE_POS_Y(35), textCopyFileButton);
    print_generic_string(ERASEFILE_X1, SCALE_POS_Y(35), textEraseFileButton);
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_end);
    // Print file names
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_menu_generic_string(SCALE_POS_X(89), SCALE_POS_Y(62), textMarioA);
    print_menu_generic_string(SCALE_POS_X(211), SCALE_POS_Y(62), textMarioB);
    print_menu_generic_string(SCALE_POS_X(89), SCALE_POS_Y(105), textMarioC);
    print_menu_generic_string(SCALE_POS_X(211), SCALE_POS_Y(105), textMarioD);
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_end);
}

#ifdef VERSION_JP
#define NOFILE_COPY_X ((SCREEN_WIDTH/2)-70)
#define COPY_FILE_X ((SCREEN_WIDTH/2)-70)
#define COPYIT_WHERE_X ((SCREEN_WIDTH/2)-70)
#define NOSAVE_DATA_X2 ((SCREEN_WIDTH/2)-70)
#define COPYCOMPLETE_X ((SCREEN_WIDTH/2)-70)
#define SAVE_EXISTS_X1 ((SCREEN_WIDTH/2)-70)
#else
#define NOFILE_COPY_X ((SCREEN_WIDTH/2)-41)
#define COPY_FILE_X ((SCREEN_WIDTH/2)-56)
#define COPYIT_WHERE_X ((SCREEN_WIDTH/2)-51)
#define NOSAVE_DATA_X2 ((SCREEN_WIDTH/2)-59)
#define COPYCOMPLETE_X ((SCREEN_WIDTH/2)-50)
#define SAVE_EXISTS_X1 ((SCREEN_WIDTH/2)-50)
#endif
    ...
    switch (messageID) {
        case COPY_MSG_MAIN_TEXT:
            if (sAllFilesExist == TRUE) {
                print_generic_string_fade(NOFILE_COPY_X, SCALE_POS_Y(197)-7, textNoFileToCopyFrom);
            } else {
                print_hud_lut_string_fade(HUD_LUT_DIFF, COPY_FILE_X, SCALE_POS_Y(43)-8, textCopyFile);
            }
            break;
        case COPY_MSG_COPY_WHERE:
            print_generic_string_fade(COPYIT_WHERE_X, SCALE_POS_Y(197)-7, textCopyItToWhere);
            break;
        case COPY_MSG_NOSAVE_EXISTS:
            print_generic_string_fade(NOSAVE_DATA_X2, SCALE_POS_Y(197)-7, textNoSavedDataExistsCopy);
            break;
        case COPY_MSG_COPY_COMPLETE:
            print_generic_string_fade(COPYCOMPLETE_X, SCALE_POS_Y(197)-7, textCopyCompleted);
            break;
        case COPY_MSG_SAVE_EXISTS:
            print_generic_string_fade(SAVE_EXISTS_X1, SCALE_POS_Y(197)-7, textSavedDataExists);
            break;
    }
}
...
#ifdef VERSION_JP
#define VIEWSCORE_X1 SCALE_POS_X(160)-27
#define ERASEFILE_X2 SCALE_POS_X(257)-37
#else
#define VIEWSCORE_X1 SCALE_POS_X(160)-32
#define ERASEFILE_X2 SCALE_POS_X(257)-27
#endif
...
static void print_copy_menu_strings(void) {
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_generic_string(RETURN_X, SCALE_POS_Y(35), textReturn);
    print_generic_string(VIEWSCORE_X1, SCALE_POS_Y(35), textViewScore);
    print_generic_string(ERASEFILE_X2, SCALE_POS_Y(35), textEraseFileButton);
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_end);
    // Print file names
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
...
#define MENU_ERASE_YES_MIN_X SCALE_POS_X(160)-15
#define MENU_ERASE_YES_MAX_X SCALE_POS_X(160)+4
#else
#define CURSOR_X (x + 0x46)
#define MENU_ERASE_YES_MIN_X SCALE_POS_X(160)-20
#define MENU_ERASE_YES_MAX_X SCALE_POS_X(160)+9
#endif

#define MENU_ERASE_YES_NO_MIN_Y SCALE_POS_Y(210)-19
#define MENU_ERASE_YES_NO_MAX_Y SCALE_POS_Y(210)
#define MENU_ERASE_NO_MIN_X SCALE_POS_X(160)+29
#define MENU_ERASE_NO_MAX_X SCALE_POS_X(160)+58
...
static void print_erase_menu_prompt(s16 x, s16 y) {
    ...
    s16 cursorY = sCursorPos[1] + (SCREEN_HEIGHT/2);
...
#ifdef VERSION_JP
#define ERASE_FILE_X SCALE_POS_X(160)-64
#define NOSAVE_DATA_X3 SCALE_POS_X(160)-70
#define MARIO_ERASED_VAR 3
#define MARIO_ERASED_X SCALE_POS_X(160)-70
#define SAVE_EXISTS_X2 SCALE_POS_X(160)-70
#else
#define ERASE_FILE_X SCALE_POS_X(160)-62
#define NOSAVE_DATA_X3 SCALE_POS_X(160)-60
#define MARIO_ERASED_VAR 6
#define MARIO_ERASED_X SCALE_POS_X(160)-60
#define SAVE_EXISTS_X2 SCALE_POS_X(160)-60
#endif
...
static void erase_menu_display_message(s8 messageID) {
    ...
    switch (messageID) {
        case ERASE_MSG_MAIN_TEXT:
            print_hud_lut_string_fade(HUD_LUT_DIFF, ERASE_FILE_X, SCALE_POS_Y(43)-8, textEraseFile);
            break;
        case ERASE_MSG_PROMPT:
            print_generic_string_fade(SCALE_POS_X(160)-70, SCALE_POS_Y(197)-7, textSure);
            print_erase_menu_prompt(SCALE_POS_X(160)-70, SCALE_POS_Y(197)-7); // YES NO, has functions for it too
            break;
        case ERASE_MSG_NOSAVE_EXISTS:
            print_generic_string_fade(NOSAVE_DATA_X3, SCALE_POS_Y(197)-7, textNoSavedDataExists);
            break;
        case ERASE_MSG_MARIO_ERASED:
            textMarioAJustErased[MARIO_ERASED_VAR] = sSelectedFileIndex + 10;
            print_generic_string_fade(MARIO_ERASED_X, SCALE_POS_Y(197)-7, textMarioAJustErased);
            break;
        case ERASE_MSG_SAVE_EXISTS: // unused
            print_generic_string_fade(SAVE_EXISTS_X2, SCALE_POS_Y(197)-7, textSavedDataExists);
            break;
    }
}
    ...
    switch (sMainMenuButtons[MENU_BUTTON_ERASE]->oMenuButtonActionPhase) {
        ...
        case ERASE_PHASE_PROMPT:
            if (update_text_fade_out() == TRUE) {
                ...
                sCursorPos[0] = 43.0f;
                sCursorPos[1] = (((f32)SCREEN_HEIGHT*80)/240);
            }
            break;
...
#ifdef VERSION_JP
#define VIEWSCORE_X2 SCALE_POS_X(160)-27
#define COPYFILE_X2 SCALE_POS_X(257)-34
#else
#define VIEWSCORE_X2 SCALE_POS_X(160)-33
#define COPYFILE_X2 SCALE_POS_X(257)-24
#endif
...
static void print_erase_menu_strings(void) {
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_generic_string(RETURN_X, SCALE_POS_Y(35), textReturn);
    print_generic_string(VIEWSCORE_X2, SCALE_POS_Y(35), textViewScore);
    print_generic_string(COPYFILE_X2, SCALE_POS_Y(35), textCopyFileButton);
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_end);
    // Print file names
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
...
#ifdef VERSION_JP
#define SOUND_SELECT_X SCALE_POS_X(160)-64
#define SOUNDMODE_X2 mode * ((74*SCREEN_HEIGHT)/240) + (SCALE_POS_X(87)-20)
#else
#define SOUND_SELECT_X SCALE_POS_X(160)-72
...
static void print_sound_mode_menu_strings(void) {
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_hud_lut_string(HUD_LUT_DIFF, SOUND_SELECT_X, SCALE_POS_Y(35), textSoundSelect);
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
    // Print mode names
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_begin);
    for (mode = 0; mode < 3; mode++) {
        ...
        textX = get_str_x_pos_from_center(mode * ((74*SCREEN_HEIGHT)/240) + SCALE_POS_X(87), textSoundModes[mode], 10.0f);
#endif
        print_generic_string(SOUNDMODE_X2, SCALE_POS_Y(87), textSoundModes[mode]);
    }
    gSPDisplayList(gDisplayListHead++, dl_ia8_text_end);
}
...
#ifdef VERSION_JP
#define MARIO_X SCALE_POS_X(0)+28
#define FILE_LETTER_X SCALE_POS_X(0)+86
#define LEVEL_NAME_X SCALE_POS_X(0)+23
#define SECRET_STARS_X SCALE_POS_X(160)-8
#define MYSCORE_X SCALE_POS_X(320)-83
#define HISCORE_X SCALE_POS_X(320)-83
#else
#define MARIO_X SCALE_POS_X(0)+25
#define FILE_LETTER_X SCALE_POS_X(0)+95
#define LEVEL_NAME_X SCALE_POS_X(0)+29
#define SECRET_STARS_X SCALE_POS_X(160)+11
#define MYSCORE_X SCALE_POS_X(320)-82
#define HISCORE_X SCALE_POS_X(320)-89
#endif
...
// TODO: EU relocates level name table to translation segment 0x19
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
    print_hud_lut_string(HUD_LUT_DIFF, MARIO_X, SCALE_POS_Y(15), textMario);
    print_hud_lut_string(HUD_LUT_GLOBAL, FILE_LETTER_X, SCALE_POS_Y(15), textFileLetter);
    // Print save file star count at top
    print_save_file_star_count(fileIndex, SCALE_POS_X(0)+124, SCALE_POS_Y(15));
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
    // Print course scores
    gSPDisplayList(gDisplayListHead++, dl_menu_ia8_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
...
#define PRINT_COURSE_SCORES(courseIndex, pad)                                                               \
    print_menu_generic_string(SCALE_POS_X(0) + 23 + (pad * 3), (SCALE_POS_Y(15)+20) + 12 * courseIndex, segmented_to_virtual(levelNameTable[courseIndex]));  \
    print_score_file_star_score(fileIndex, courseIndex, SCALE_POS_X(160)-8, (SCALE_POS_Y(15)+20) + 12 * courseIndex);                        \
    print_score_file_course_coin_score(fileIndex, courseIndex, SCALE_POS_X(320)-107, (SCALE_POS_Y(15)+20) + 12 * courseIndex);
#else
#define PADCHAR 1
#define PRINT_COURSE_SCORES(courseIndex, pad)                                                               \
    print_menu_generic_string(SCALE_POS_X(0) + 23 + (pad * 3), (SCALE_POS_Y(15)+20) + 12 * courseIndex, segmented_to_virtual(levelNameTable[courseIndex]));  \
    print_score_file_star_score(fileIndex, courseIndex, SCALE_POS_X(160)+11, (SCALE_POS_Y(15)+20) + 12 * courseIndex);                        \
    print_score_file_course_coin_score(fileIndex, courseIndex, SCALE_POS_X(320)-107, (SCALE_POS_Y(15)+20) + 12 * courseIndex);
#endif
...
#undef PADCHAR
    ...
    // Print level name
    print_menu_generic_string(LEVEL_NAME_X, SCALE_POS_Y(15)+200, segmented_to_virtual(levelNameTable[25]));
    // Print castle secret stars
    print_score_file_castle_secret_stars(fileIndex, SECRET_STARS_X, SCALE_POS_Y(15)+200);
    // Print current coin score mode
    if (sScoreFileCoinScoreMode == 0) {
        print_menu_generic_string(MYSCORE_X, SCALE_POS_Y(15)+9, textMyScore);
    } else {
        print_menu_generic_string(HISCORE_X, SCALE_POS_Y(15)+9, textHiScore);
    }
...
void lvl_init_menu_values_and_cursor_pos(UNUSED s32 arg, UNUSED s32 unused) {
    ...
    switch (gCurrSaveFileNum) {
        case 1: // File A
            sCursorPos[0] = -(((f32)SCREEN_HEIGHT*94)/240);
            sCursorPos[1] = (((f32)SCREEN_HEIGHT*46)/240);
            break;
        case 2: // File B
            sCursorPos[0] = ((f32)SCREEN_HEIGHT/10);
            sCursorPos[1] = (((f32)SCREEN_HEIGHT*46)/240);
            break;
        case 3: // File C
            sCursorPos[0] = -(((f32)SCREEN_HEIGHT*94)/240);
            sCursorPos[1] = (((f32)SCREEN_HEIGHT*5)/240);
            break;
        case 4: // File D
            sCursorPos[0] = ((f32)SCREEN_HEIGHT/10);
            sCursorPos[1] = (((f32)SCREEN_HEIGHT*5)/240);
