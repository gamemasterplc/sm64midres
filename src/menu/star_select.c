...
static void print_course_number(void) {
    ...
    create_dl_translation_matrix(MENU_MTX_PUSH, ((SCREEN_WIDTH/2)-2), 81.0f, 0.0f);
    ...
    if (gCurrCourseNum < 10) { // 1 digit number
        print_hud_lut_string(HUD_LUT_GLOBAL, ((SCREEN_WIDTH/2)-8), (SCREEN_HEIGHT-82), courseNum);
    } else { // 2 digit number
        print_hud_lut_string(HUD_LUT_GLOBAL, ((SCREEN_WIDTH/2)-17), (SCREEN_HEIGHT-82), courseNum);
    }
...
#ifdef VERSION_JP
#define ACT_NAME_X ((SCREEN_WIDTH/2)-2)
#else
#define ACT_NAME_X ((SCREEN_WIDTH/2)+3)
#endif
...
// TODO: EU relocates level and act name tables to translation segment 0x19
    ...
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, 255);
    print_hud_my_score_coins(1, gCurrSaveFileNum - 1, gCurrCourseNum - 1, ((SCREEN_WIDTH/2)-5), ((SCREEN_HEIGHT/2)-14));
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
    ...
    if (save_file_get_course_coin_score(gCurrSaveFileNum - 1, gCurrCourseNum - 1) != 0) {
        print_generic_string(((SCREEN_WIDTH/2)-58), ((SCREEN_HEIGHT/2)-2), myScore);
    }
    // Print the level name; add 3 to skip the number and spacing to get to the actual string to center.
    lvlNameX = get_str_x_pos_from_center((SCREEN_WIDTH/2), currLevelName + 3, 10.0f);
    ...
    if (sVisibleStars != 0) {
        ...
        print_menu_generic_string(actNameX, ((SCREEN_HEIGHT*81)/240), selectedActName);
    }
    ...
    for (i = 1; i <= sVisibleStars; i++) {
        starNumbers[0] = i;
		#if SCREEN_HEIGHT == 240
        print_menu_generic_string(i * 34 - sVisibleStars * 17 + ((SCREEN_WIDTH/2)-21), 38, starNumbers);
		#else
		aspect = (f32)SCREEN_WIDTH/(f32)SCREEN_HEIGHT;
		number_pos = i * 34 - sVisibleStars * 17 + ((aspect*120)-21);
		print_menu_generic_string(((number_pos*SCREEN_HEIGHT)/240), ((SCREEN_HEIGHT*38)/240), starNumbers);
		#endif
    }
